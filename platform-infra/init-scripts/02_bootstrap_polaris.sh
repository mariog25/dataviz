...
# Normalize URI value to avoid common WSL/.env issues (quotes, CRLF, wrong path).
POLARIS_URI=$(printf '%s' "${POLARIS_URI}" | tr -d '' | sed -E 's/^"(.*)"$//' | sed -E "s/^'(.*)'$//")
# If user passed REST URI (.../api/catalog), convert to Polaris base URI automatically.
POLARIS_URI=${POLARIS_URI%/api/catalog}
POLARIS_URI=${POLARIS_URI%/}

if [[ ! "${POLARIS_URI}" =~ ^https?:// ]]; then
  echo "[polaris] ERROR: POLARIS_URI must include scheme, e.g. http://localhost:8181"
  echo "[polaris] Current value: ${POLARIS_URI}"
  exit 1
fi
...
echo "[polaris] resolved POLARIS_URI=${POLARIS_URI}"
start_ts=$(date +%s)
